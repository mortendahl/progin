...
public class TelephoneWords {
	 ...
	 * 
	 * Solution: recursively explore tree where a node at depth i has branches
	 * corresponding to the character at position i - note that all sub-trees
	 * are identical (suggesting that bottom-up is better -- fewer method calls)
	 * - similar to combinations, but now the branches are not include/exclude
	 * but the possible translations
	 */
	...
	public static void main(String[] args) {
		...
		// assume they all have the same length; otherwise we may get index out
		// of bounds exception
	...
	private static char[][] keyMap = { { '0' }, // 0
			{ '1' }, // 1
			{ 'a', 'b', 'c' }, // 2
			{ 'd', 'e', 'f' }, // 3
			{ 'g', 'h', 'f' }, // 4
	...
	public static List<String> translationsBottomUp(String input) {
		...
		if (input.length() == 0) {
			...
			// recursive call may be outside loop since all sub-trees are
			// identical
	...
	private static void translationsTopDown(String input, String output,
			List<String> acc) {
		if (input.length() == 0) {
			// base case
			acc.add(output);
